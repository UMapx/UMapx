    ...
    public static class Schur
    {
        ...
        private static (float[,] Q, float[,] T) Factor(float[,] A, float eps)
        {
            ...
            for (int row = 0; row < n; row++)
                for (int column = 0; column < n; column++)
                    // Roundoff below the first subdiagonal must not leak into the Schur form.
                    t[row, column] = column < row - 1 ? 0 : (float)(hessenberg[row][column] * inputScale);
            return (InternalMatrixMath.Real(matrices), t);
        }

        /// <summary>Uses implicit shifted QR on a complex Hessenberg matrix, matching the real reduction strategy.</summary>
        ...
        internal static (C[,] Q, C[,] T) Factor(C[,] a, double eps = 1e-16, int iterations = 1000)
        {
            ...
            while (high > 0)
            {
                ...
                // Complex single-shift specialization of the real implicit QR:
