    ...
    public class LocalLaplacianFilter : IFilter
    {
        ...
        private static void Llfilter(float[,] input, int radius, float sigma, float factor, int n, int levels)
        {
            ...
            for (float i = min; i <= max; i += step)
            {
                ...
                for (y = 0; y < height; y++)
                {
                    for (x = 0; x < width; x++)
                    {
                        I_temp[y, x] = T[MathF.Byte(input[y, x] * (length - 1))];
                    }
                }
                ...
                for (level = 0; level < n_levels; level++)
                {
                    ...
                    for (y = 0; y < height; y++)
                    {
                        for (x = 0; x < width; x++)
                        {
                            I_outp[y, x] += T[MathF.Byte(I_gaus[y, x] * (length - 1))] * I_temp[y, x];
                        }
                    }
        ...
        private static void Llfilter(float[] input, int radius, float sigma, float factor, int n, int levels)
        {
            ...
            for (float i = min; i <= max; i += step)
            {
                ...
                for (y = 0; y < height; y++)
                {
                    I_temp[y] = T[MathF.Byte(input[y] * (length - 1))];
                }
                ...
                for (level = 0; level < n_levels; level++)
                {
                    ...
                    for (y = 0; y < height; y++)
                    {
                        I_outp[y] += T[MathF.Byte(I_gaus[y] * (length - 1))] * I_temp[y];
                    }
        ...
        private static float Rem(float x, float sigma, float factor, float i)
        {
            ...
            float y = x - i;
            return factor * y * MathF.Exp(-y * y / z);
        }
        /// <summary>
        /// Remapping function.
        /// </summary>
