...
public class WindowedTransformTests
{
    public static IEnumerable<object[]> WeylMatrixCases()
    {
    ...
    [Theory]
    [MemberData(nameof(WeylMatrixCases))]
    public void RectangularWeylTransformsAgreeAndInvertInEveryDirection(string kind, Direction direction)
    {
        const int n = 16, m = 24, shift = 4;
        var window = new Hamming(shift);
        var x = Matrix(n, m);
        var z = new Complex32[n, m];
        for (int i = 0; i < n; i++)
            for (int j = 0; j < m; j++)
                z[i, j] = new Complex32(x[i, j], (float)Math.Cos(.3 * i + .2 * j));
        ...
        )
        {
            ...
            for (int i = 0; i < n; i++)
                for (int j = 0; j < m; j++)
                    Close((Complex)z[i, j], r[i, j], .001);
            if (kind != "Complex")
                Close(x, d.Backward(d.Forward(x)), .001);
        }
    }
    ...
    {
        var window = new Hamming(frame);
        var x = Enumerable.Range(0, frame * 3).Select(i => new Complex32((float)Math.Sin(i * .31), (float)Math.Cos(i * .27))).ToArray();
        ITransform d = fast ? new FastShortTimeFourierTransform(window) : new ShortTimeFourierTransform(window);
        ...
        for (int block = 0; block < 3; block++)
            for (int k = 0; k < frame; k++)
            {
                ...
                for (int j = 0; j < frame; j++)
                    expected += (Complex)x[block * frame + j] * (.001 + .53836 - .46164 * Math.Cos(2 * Math.PI * j / (frame - 1))) * Complex.Exp(-2 * Math.PI * Complex.ImaginaryOne * j * k / frame) / Math.Sqrt(frame);
                Close(expected, actual[block * frame + k], 1e-4);
            }

        var restored = d.Backward(actual);
        for (int i = 0; i < x.Length; i++)
            Close((Complex)x[i], restored[i], 1e-4);
    ...
    public void ZakTransformMatchesItsFiniteSumAndInverse(int n, int m, bool fast)
    {
        ...
        IZakTransform d = fast ? new FastZakTransform(m) : new ZakTransform(m);
        var actual = d.Forward(x);
        var real = d.Forward(x.Select(z => z.Real).ToArray());
        int l = n / m;
        Assert.Equal(n, actual.GetLength(0));
        Assert.Equal(l, actual.GetLength(1));
        for (int row = 0; row < n; row++)
            for (int k = 0; k < l; k++)
            {
                ...
                for (int j = 0; j < l; j++)
                {
                    int index = ((row - m * j) % n + n) % n;
                    var phase = Complex.Exp(-Complex.ImaginaryOne * 2 * Math.PI * j * k / l);
                    expected += (Complex)x[index] * phase;
                    expectedReal += x[index].Real * phase;
                }

                Close(expected, actual[row, k], 1e-4);
                Close(expectedReal, real[row, k], 1e-4);
            }
    ...
    public void WeylHeisenbergFastAndDirectBasesAgreeAndReconstruct(int n, int m, string kind)
    {
        ...
        var window = new Hamming(m);
        var x = Enumerable.Range(0, n).Select(i => (float)Math.Sin(i * .31)).ToArray();
        var z = x.Select((v, i) => new Complex32(v, (float)Math.Cos(i * .19))).ToArray();
        bool real = kind.StartsWith("Real");
        var spectrum = kind.Contains("Hartley") ? SpectrumType.Hartley : SpectrumType.Fourier;
        ...
        {
            var d = kind.EndsWith("Fast") ? fast : direct;
            var r = d.Backward(d.Forward(z));
            for (int i = 0; i < n; i++)
                Close((Complex)z[i], r[i], .002);
            if (real)
                Close(x, d.Backward(d.Forward(x)), .002);
            return;
        }

        var expected = direct.Forward(z);
        var actual = fast.Forward(z);
        Assert.Equal(expected.Length, actual.Length);
        for (int i = 0; i < actual.Length; i++)
            Close((Complex)expected[i], actual[i], .001);
        if (real)
            Close(direct.Forward(x), fast.Forward(x), .001);
    }
}
