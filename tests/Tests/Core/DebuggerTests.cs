...
public class DebuggerTests
{
    ...
    public void NullMatrixCellsAndNullTextArePrintedAsEmptyCells(bool jagged)
    {
        ...
        {
            if (jagged)
                Debugger.Print(new object?[][] { new object?[] { null, "x" }, new object?[] { "y", new NullText() } });
            else
                Debugger.Print(new object?[,] { { null, "x" }, { "y", new NullText() } });
        });
    ...
    public void WideMatricesSplitIntoBlocksAndKeepColumnAlignment(bool jagged, bool oversized)
    {
        ...
        {
            if (jagged)
                Debugger.Print(new[] { new[] { longValue, "x", "yy" }, new[] { "z", "tt", "q" } });
            else
                Debugger.Print(new[,] { { longValue, "x", "yy" }, { "z", "tt", "q" } });
        });
        Assert.Equal(expected, actual);
    }
    ...
    public static IEnumerable<object[]> InfoOptions()
    {
        foreach (bool inherited in new[]
        {
            ...
            foreach (bool statics in new[]
            {
                ...
                foreach (bool signatures in new[]
                {
                    false,
                    true
                }
    ...
    {
        CheckInfo(Capture(() => Debugger.Info(new InfoSample(), inherited, statics, signatures)), inherited, statics, signatures);
    }
