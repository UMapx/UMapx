...
public class SpecialFunctionTests
{
    ...
    public void BesselWronskianHoldsAtComplexArgument()
    {
        ...
        int n = 20;
        Complex left = (Complex)Special.J(z, n + 1) * (Complex)Special.Y(z, n) - (Complex)Special.J(z, n) * (Complex)Special.Y(z, n + 1);
    ...
    public void MinkowskiFixesIntegerArguments()
    {
        foreach (long n in new[]
        {
            ...
            Assert.Equal((float)n, Special.Minkowski(n));
    }

    [Fact]
    public void GammaSeriesMustIncludeGammaNormalization() => Close(0.5595067149347877, Special.GammaP(5, 5));
    [Fact]
    public void GammaContinuedFractionMustUseNextDenominator() => Close(4 * Math.Exp(-3), Special.GammaQ(2, 3));
    [Fact]
    public void ComplexErfAgreesWithEntireFunctionReference() => Close(new Complex(-1.0035022433130363, .004740903031294336), Special.Erf(new Complex32(-2, .5f)));
    [Fact]
    public void LambertWReturnsRequestedPrincipalBranch() => Close(new Complex(.21453028412564626, .3510955496497862), Special.LambertW(new Complex32(.1f, .5f), 0));
    [Fact]
    public void BesselYIncludesBothExponentialTerms() => Close(.08825696421567696, Special.Y(1f, 0));
    [Fact]
    public void BesselJAsymptoticAccuracyDependsOnOrder() => Close(.14853180559607407, Special.J(21f, 10));
    [Fact]
    public void BesselJAtImaginaryArgumentSatisfiesConnectionToI() => Close(new Complex(-145831809975.96713, 0), Special.J(new Complex32(0, 30), 10));
    [Fact]
    public void ComplexBesselKResolvesOscillatoryIntegral() => Close(new Complex(3.015242162785077, -31.062215312887222), Special.K(new Complex32(1, 5), 10));
    [Fact]
    public void BetaAvoidsIntermediateGammaOverflow() => Close(1.7681885473062026e-15, Special.Beta(20f, 30f), 1e-20);
    [Fact]
    public void HypergeometricSeriesAvoidsIntermediatePochhammerOverflow() => Close(21.789416887313024, Special.Hypergeom(2f, 3f, 4f, .9f));
    [Fact]
    public void ChebyshevUHasFiniteEndpointValue() => Close(3, Special.ChebyshevU(1f, 2));
    [Fact]
    public void ChebyshevPolynomialIsDefinedOutsideUnitInterval() => Close(7, Special.ChebyshevT(2f, 2));
    [Fact]
    public void PositiveOrderPochhammerOfZeroIsZero() => Close(0, Special.FactorialUp(0, 2));
}
