...
public class RectangleTests
{
    ...
    private static (int X, int Y, int Width, int Height)[] IntegerCases(int seed = 731)
    {
        var cases = new List<(int X, int Y, int Width, int Height)>
        {
            ...
            (5, 5, 0, 4),
            (5, 5, 4, 0),
            (int.MaxValue, int.MinValue, 4, -5),
            (int.MinValue, int.MaxValue, -4, 5),
            (-1, -2, int.MaxValue, int.MinValue)
        };
        var random = new Random(seed);
        for (int i = 0; i < 60; i++)
            cases.Add((random.Next(-50, 50), random.Next(-50, 50), random.Next(-10, 70), random.Next(-10, 70)));
        for (int i = 0; i < 64; i++)
            cases.Add(((int)random.NextInt64(int.MinValue, (long)int.MaxValue + 1), (int)random.NextInt64(int.MinValue, (long)int.MaxValue + 1), (int)random.NextInt64(int.MinValue, (long)int.MaxValue + 1), (int)random.NextInt64(int.MinValue, (long)int.MaxValue + 1)));
    ...
    private static (float X, float Y, float Width, float Height)[] FloatCases(int seed = 731)
    {
        ...
        cases.AddRange(new (float X, float Y, float Width, float Height)[] { (-2.75f, 3.125f, 4.5f, 1.75f), (-0.0f, 0, 0, -0.0f), (float.Epsilon, -float.Epsilon, float.Epsilon, float.Epsilon), (float.MaxValue, float.MinValue, float.MaxValue, float.MaxValue), (float.PositiveInfinity, 0, 2, 3), (0, float.NegativeInfinity, 2, 3), (0, 0, float.PositiveInfinity, 3), (float.NaN, 0, 2, 3), (0, float.NaN, 2, 3), (0, 0, float.NaN, 3), (0, 0, 2, float.NaN) });
        var random = new Random(seed);
        for (int i = 0; i < 64; i++)
        {
    ...
    public void IntegerConstructionPropertiesAndEqualityPreserveComponents()
    {
        foreach (var expected in IntegerCases())
        {
            var actual = new RectangleInt(new PointInt(expected.X, expected.Y), new SizeInt(expected.Width, expected.Height));
            ...
            Same((expected.X, expected.Y, unchecked(expected.Width - expected.X), unchecked(expected.Height - expected.Y)), RectangleInt.FromLTRB(expected.X, expected.Y, expected.Width, expected.Height));
    ...
    public void FloatConstructionPropertiesAndEqualityPreserveComponents()
    {
        foreach (var expected in FloatCases())
        {
            var actual = new RectangleFloat(new PointFloat(expected.X, expected.Y), new SizeFloat(expected.Width, expected.Height));
            Same(expected, actual);
            Assert.Equal(expected.X, actual.Left);
            Assert.Equal(expected.Y, actual.Top);
            Assert.Equal(unchecked(expected.X + expected.Width), actual.Right);
            ...
            Same((expected.X, expected.Y, expected.Width - expected.X, expected.Height - expected.Y), RectangleFloat.FromLTRB(expected.X, expected.Y, expected.Width, expected.Height));
            bool reflexive = !float.IsNaN(expected.X) && !float.IsNaN(expected.Y) && !float.IsNaN(expected.Width) && !float.IsNaN(expected.Height);
    ...
    public void IntegerOffsetAndInflationWrapOverflowInEachComponent()
    {
        foreach (var rectangle in IntegerCases())
            ...
            )
            {
                ...
                var expected = (X: unchecked(rectangle.X - amount), Y: unchecked(rectangle.Y + 3), Width: unchecked(rectangle.Width + 2 * amount), Height: unchecked(rectangle.Height - 6));
    ...
    public void FloatOffsetAndInflationUseEachComponent()
    {
        foreach (var rectangle in FloatCases())
            ...
            )
            {
                ...
                var expected = (X: rectangle.X - amount, Y: rectangle.Y + 3.5f, Width: rectangle.Width + 2 * amount, Height: rectangle.Height - 7);
    ...
    public void StringRepresentationUsesCurrentCulture(string culture, string expected)
    {
        ...
        try
        {
            ...
            Assert.Equal(expected, new RectangleFloat(-3.25f, 4.5f, 5.75f, 6.125f).ToString());
        }
        finally
        {
    ...
    private static void SameBits(float expected, float actual) => Assert.Equal(BitConverter.SingleToInt32Bits(expected), BitConverter.SingleToInt32Bits(actual));
    [Fact]
