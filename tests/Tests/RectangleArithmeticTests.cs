...
public class RectangleArithmeticTests
{
    ...
    {
        var rectangles = new[] { new RectangleInt(10, 20, 3, 4), new RectangleInt(-10, -20, 6, 2) };
    ...
    {
        var rectangles = new[] { new RectangleFloat(10.25f, 20.5f, 3, 4), new RectangleFloat(-10.25f, -20.5f, 6, 2) };
    ...
    public void CornersAreClockwiseAndFromPointsUsesOnlyFirstAndThirdPoints()
    {
        ...
        Assert.Equal(new[] { new PointInt(10, 20), new PointInt(13, 20), new PointInt(13, 24), new PointInt(10, 24) },
            rectangle.ToPoints());
        Assert.Equal(rectangle, RectangleInt.FromPoints(new[] { new PointInt(10, 20), new PointInt(-99, 99),
            new PointInt(13, 24), new PointInt(99, -99) }));

        var floating = new RectangleFloat(10.25f, 20.5f, 3.5f, 4.25f);
        Assert.Equal(new PointFloat(10.25f, 20.5f), floating.GetPoint());
        Assert.Equal(new[] { new PointFloat(10.25f, 20.5f), new PointFloat(13.75f, 20.5f),
            new PointFloat(13.75f, 24.75f), new PointFloat(10.25f, 24.75f) }, floating.ToPoints());
        Assert.Equal(floating, RectangleFloat.FromPoints(new[] { new PointFloat(10.25f, 20.5f), new PointFloat(-99, 99),
            new PointFloat(13.75f, 24.75f), new PointFloat(99, -99) }));
    }
    ...
    public void ClampNormalizesNegativeDimensionsAndRetainsDisjointLocation(
        int x, int y, int width, int height, int expectedX, int expectedY, int expectedWidth, int expectedHeight)
    {
        var rectangle = new RectangleInt(x, y, width, height);
        Assert.Equal(new RectangleInt(expectedX, expectedY, expectedWidth, expectedHeight),
            rectangle.Clamp(new RectangleInt(0, 0, 10, 10)));
        Assert.Equal(new RectangleInt(x, y, width, height), rectangle);
        var floating = new RectangleFloat(x + 0.25f, y + 0.5f, width, height);
        Assert.Equal(new RectangleFloat(expectedX + 0.25f, expectedY + 0.5f, expectedWidth, expectedHeight),
            floating.Clamp(new RectangleFloat(0.25f, 0.5f, 10, 10)));
