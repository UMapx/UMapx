...
public class RangeTests
{
    ...
    private static void Same(float expected, float actual) => Assert.Equal(BitConverter.SingleToInt32Bits(expected), BitConverter.SingleToInt32Bits(actual));
            ...
            for (int max = -3; max <= 3; max++)
            {
                ...
                for (int otherMin = -3; otherMin <= 3; otherMin++)
                    for (int otherMax = -3; otherMax <= 3; otherMax++)
                    {
                        ...
                        var expectedIntersection = common.Length == 0 ? RangeInt.Empty : new RangeInt(common.Min(), common.Max());
                        var expectedUnion = combined.Length == 0 ? RangeInt.Empty : new RangeInt(combined.Min(), combined.Max());
            ...
            foreach (float max in values)
            {
                ...
                foreach (float otherMin in values)
                    foreach (float otherMax in values)
                    {
                        ...
                        var common = first.Intersect(second).ToArray();
                        var combined = first.Concat(second).ToArray();
                        var expectedIntersection = common.Length == 0 ? RangeFloat.Empty : new RangeFloat(common.Min(), common.Max());
                        var expectedUnion = combined.Length == 0 ? RangeFloat.Empty : new RangeFloat(combined.Min(), combined.Max());
                        Assert.Equal(common.Length > 0, a.IntersectsWith(b));
                        Assert.Equal(first.Length > 0 && second.Length > 0 && second.All(first.Contains), a.Contains(b));
    ...
    public void FloatOperationsHandleNaNInfinityAndSignedZero()
    {
        ...
        {
            new RangeFloat(float.NaN, 1),
            new RangeFloat(0, float.NaN)
        }
