...
public class RectangleGeometryTests
{
    ...
    private static HashSet<(int X, int Y)> Grid(int x, int y, int width, int height) =>
        (from column in Enumerable.Range(x, width)
         from row in Enumerable.Range(y, height)
    ...
    private static RectangleInt Integer((int X, int Y, int Width, int Height) r) =>
        new(r.X, r.Y, r.Width, r.Height);

    private static RectangleFloat Floating((int X, int Y, int Width, int Height) r, float scale = 1) =>
    ...
    public void EmptyNegativeAndTouchingBoundsPreserveSpecifiedGeometry(int x, int y, int width, int height,
        bool contains, bool overlaps, int ix, int iy, int iw, int ih, int uw, int uh)
    {
    ...
    {
        RectangleInt Create(int x, int width) => transpose
            ? new RectangleInt(0, x, 10, width) : new RectangleInt(x, 0, width, 10);
    ...
    public void NaNComponentsPreventContainmentAndPropagateThroughUnion(int component,
    ...
    public void InfiniteBoundsHaveSpecifiedIntersectionsAndUnions(float x, float width, bool overlaps,
        float ix, float iy, float iw, float ih, float ux, float uw)
    {
