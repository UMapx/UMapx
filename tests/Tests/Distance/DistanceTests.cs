...
public class DistanceTests
{
    ...
    public void NumericDistancesAgreeWithDefinitions(string name, int n)
    {
        ...
        var pm = new float[2, n];
        var qm = new float[2, n];
        for (int j = 0; j < n; j++)
        {
            pm[0, j] = p[j];
            qm[0, j] = q[j];
        ...
        foreach (float value in distance.Compute(pm, qm))
            Close(expected, value, 5e-4);
    }
    ...
    public void ComplexDistancesAgreeWithHermitianDefinitions(string name, int n)
    {
        ...
        var pm = new Complex32[2, n];
        var qm = new Complex32[2, n];
        for (int j = 0; j < n; j++)
        {
            pm[0, j] = p[j];
            qm[0, j] = q[j];
        ...
        foreach (var value in distance.Compute(pm, qm))
            Close(new Complex(expected, 0), value, 5e-4);
    }

    [Theory, MemberData(nameof(BooleanCases))]
    public void BooleanDistancesUseContingencyCounts(string name, int seed)
    {
        var pairs = new List<(bool A, bool B)>
        {
            (true, true),
            (true, false),
            (false, true),
        ...
        var p = pairs.Select(x => x.A ? 1f : 0f).ToArray();
        var q = pairs.Select(x => x.B ? 1f : 0f).ToArray();
        var distance = Create(name);
    ...
    public void SokalSneathMatchesIndependentContingencyCounts(int tt, int tf, int ft)
    {
        ...
        Close(expected, distance.Compute(p, q));
        Close(expected, distance.Compute(q, p));
        var pc = p.Select(x => new Complex32(0, x)).ToArray();
        var qc = q.Select(x => new Complex32(0, x)).ToArray();
        RealComplex(expected, distance.Compute(pc, qc));
        RealComplex(expected, distance.Compute(qc, pc));
    }
