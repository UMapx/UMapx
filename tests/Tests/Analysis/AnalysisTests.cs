...
public class AnalysisTests
{
    ...
    public void QuadratureAgreesWithAnalyticAntiderivatives(IntegrationMethod method, bool reverse, bool complex)
    {
        ...
        var q = new Integration(method);
        Assert.Equal(method, q.MethodType);
        ...
        a = (Complex)(Complex32)a;
        b = (Complex)(Complex32)b;
    ...
    public static IEnumerable<object[]> DifferentiationCases()
    {
        foreach (int points in new[]
        {
            ...
            foreach (int order in Enumerable.Range(0, points + 1))
                foreach (bool complex in new[]
                {
                    false,
                    true
    ...
    {
        var d = new Differentiation(points);
        Assert.Equal(points, d.Points);
    ...
    [Theory]
    [InlineData(NonlinearMethod.Bisection)]
    [InlineData(NonlinearMethod.Chord)]
    [InlineData(NonlinearMethod.Secant)]
    [InlineData(NonlinearMethod.FalsePosition)]
    ...
    public void GoldenSectionFindsTheArgumentOfTheExtremum(bool maximum)
    {
        ...
        search.Eps = 1e-4f;
        Close(1e-4, search.Eps);
    }

    public static IEnumerable<object[]> OdeCases()
    {
        foreach (var method in Enum.GetValues<DifferentialMethod>())
            foreach (bool irregular in new[]
            {
                false,
                true
            }
    ...
    public void OdeSolversFollowTheAnalyticExponentialOnUniformAndNonuniformGrids(DifferentialMethod method, bool irregular, bool complex)
    {
        ...
        if (complex)
        {
            ...
            for (int i = 0; i < y.Length; i++)
                Close(Complex.Exp(nodes[i + 1]), y[i], tolerance);
        }
        else
        {
            ...
            Assert.Equal(x.Length - 1, y.Length);
            for (int i = 0; i < y.Length; i++)
                Close(Math.Exp(x[i + 1]), y[i], tolerance);
        }
    }
    ...
    public void AdamsBashforthIntegratesPolynomialRightHandSides(int order)
    {
        ...
        for (int p = 0; p < order; p++)
            Close(1.0 / (p + 1), Enumerable.Range(0, order).Sum(j => c[j] * Math.Pow(-j, p)), 2e-5);
    ...
    {
        foreach (var method in Enum.GetValues<InterpolationMethod>())
            foreach (float query in new[]
            {
                -1f,
                0,
                .3f,
    ...
    public void InterpolationReproducesPolynomialsAtNodesAndBetweenThem(InterpolationMethod method, float query, bool complex)
    {
        ...
        int degree = method == InterpolationMethod.Linear ? 1 : 3;
    ...
    [Theory]
    [InlineData(ApproximationMethod.Polynomial)]
    [InlineData(ApproximationMethod.Logarithmic)]
    [InlineData(ApproximationMethod.Exponential)]
    [InlineData(ApproximationMethod.Power)]
    public void LeastSquaresRecoversExactModelFamilies(ApproximationMethod method)
    {
        ...
        var a = new Approximation(1, method);
        Close(y, a.Compute(x, y), 1e-4);
        Close(y, a.Compute(x, y, out var c), 1e-4);
        Close(new[] { 1f, .75f }, c, 1e-4);
        Close(y, a.Compute(x, y, out c, out var fit), 1e-4);
        Close(1, fit, 2e-5);
        Close(y, a.Compute(x, y, out c, out fit, out var equation), 1e-4);
        Assert.False(string.IsNullOrWhiteSpace(equation));
        var cx = x.Select(t => new Complex32(t, 0)).ToArray();
        var cy = y.Select(t => new Complex32(t, 0)).ToArray();
        var results = new[]
        {
            a.Compute(cx, cy),
            a.Compute(cx, cy, out var cc),
            a.Compute(cx, cy, out cc, out var cf),
            a.Compute(cx, cy, out cc, out cf, out var ce)
        };
        foreach (var result in results)
            for (int i = 0; i < x.Length; i++)
                Close((Complex)cy[i], result[i], 2e-4);
        Close(new Complex(1, 0), cc[0], 2e-4);
        Close(new Complex(.75, 0), cc[1], 2e-4);
        Close(new Complex(1, 0), cf, 2e-4);
    ...
    public void RootsSatisfyPolynomialsAndRecoverTheirCoefficients(int degree)
    {
        ...
        foreach (var r in expected)
        {
            ...
            for (int i = 0; i < coefficients.Length; i++)
            {
                ...
                next[i + 1] -= r.Real * coefficients[i];
            }

            coefficients = next;
        }

        var solver = new Roots(1e-7f);
        var p = coefficients.Select(t => (float)t).ToArray();
        ...
        {
            var actual = solver.Compute(padded).OrderBy(z => z.Real).ToArray();
            Assert.Equal(degree, actual.Length);
            for (int i = 0; i < degree; i++)
                Close((Complex)expected[i], actual[i], .002);
        }
    }
    ...
    public void PadeCoefficientsMatchTheTaylorSeriesThroughTheRequestedOrder(int m, int n)
    {
        ...
        Check(p, q);
        var (cp, cq) = d.Compute(t.Select(v => new Complex32(v, 0)).ToArray());
        ...
        )
        {
            ...
            Close(ratio, d.Compute(x, p, q));
            Close(new Complex(ratio, 0), d.Compute(new Complex32(x, 0), p, q));
            Close(new Complex(ratio, 0), d.Compute(x, cp, cq));
            Close(new Complex(ratio, 0), d.Compute(new Complex32(x, 0), cp, cq));
        }

        Assert.False(string.IsNullOrWhiteSpace(d.Equation(p, q)));
        Assert.False(string.IsNullOrWhiteSpace(d.Equation(cp, cq)));
    }
    ...
    public void PadeReconstructsIndependentTaylorProductEquations(bool complex, int m, int n)
    {
        ...
        var coefficients = new Complex32[m + n + 1];
        coefficients[0] = 1;
        Complex term = 1;
        for (int k = 1; k < coefficients.Length; k++)
        {
            term *= s / k;
            coefficients[k] = (Complex32)term;
        }

        var pade = new Pade(m, n);
        Complex32[] p, q;
        if (complex)
            (p, q) = pade.Compute(coefficients);
        else
        {
        ...
        Assert.Equal(m + 1, p.Length);
        Assert.Equal(n + 1, q.Length);
        Assert.Equal(1, q[0].Real);
        for (int k = 0; k <= m + n; k++)
        {
            Complex product = 0;
            for (int j = 0; j <= Math.Min(k, n); j++)
                product += (Complex)q[j] * (Complex)coefficients[k - j];
    ...
    [InlineData(0f)]
    [InlineData(1f)]
    public void BilinearInterpolationIsLinearAlongGridEdges(float x) => Close(x + .5, new Interpolation().Compute(new[] { 0f, 1f }, new[] { 0f, 1f }, new float[,] { { 0, 1 }, { 1, 2 } }, x, .5f));
    [Theory]
    [InlineData(2)]
    [InlineData(3)]
    [InlineData(5)]
    ...
    [Theory]
    [InlineData(InterpolationMethod.Lagrange)]
    [InlineData(InterpolationMethod.Newton)]
    [InlineData(InterpolationMethod.Barycentric)]
