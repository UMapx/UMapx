...
public class WindowTests
{
    ...
    public void SamplesAgreeWithIndependentWindowFormulas(string name, int n)
    {
        ...
        for (int i = 0; i < n; i++)
            Close(Reference(name, i, n), actual[i], 5e-5, 5e-5);
    }
    ...
    public void ExplicitFrameSizeDoesNotDependOnStoredFrameSize(string name, int n)
    {
        ...
        var window = Create(name, n + 4);
        var target = Create(name, n);
        if (window is Confined oldConfined && target is Confined targetConfined)
            oldConfined.Sigma = targetConfined.Sigma;
        var expected = target.GetWindow();
        var actual = window.GetWindow(n);
        ...
        for (int i = 0; i < expected.Length; i++)
            if (float.IsNaN(expected[i]))
                ...
                Close(expected[i], actual[i], 1e-5);
    }
    ...
    [Fact]
    public void EvenNormalWindowIsSymmetric()
    {
        var w = new Normal(8).GetWindow();
        Close(w, w.Reverse().ToArray());
    }

    [Fact]
    public void EvenConfinedWindowIsSymmetric()
    {
        var w = new Confined(8).GetWindow();
        Close(w, w.Reverse().ToArray());
    }
}
