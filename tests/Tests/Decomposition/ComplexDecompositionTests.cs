...
public class ComplexDecompositionTests
{
    ...
    public void IsolatedLargeEigenvalueDoesNotCorruptSmallBlockEigenvectors(float large)
    {
        ...
        foreach (var (v, values) in new[]
        {
            (eigen.V, eigen.D),
            (generalized.V, GEVD.Eigenvalues(generalized.Alpha, generalized.Beta))
        }
    ...
    public void GsvdResolvesSmallCoupledBlocksBesideLargeSingularValues(float small, bool real, bool swap)
    {
        ...
        if (real)
        {
            var ra = new float[3, 3];
            var rb = new float[3, 3];
            for (int i = 0; i < 3; i++)
                for (int j = 0; j < 3; j++)
                {
                    ra[i, j] = a[i, j].Real;
        ...
        for (int i = 0; i < 3; i++)
            NumericAssert.Close(expected[i], actual[i], 0, 2e-5);
    }
        ...
        foreach (bool complex in new[]
        {
            ...
            )
            {
                ...
                if (!complex)
                {
                    ...
                    for (int j = 0; j < 2; j++)
                        NumericAssert.Close(d.S2[j], real.S2[j], 0, 2e-6);
                }
            }
    }
    ...
    public void RealEigenvaluesPreserveScaleIncludingAtZeroTolerance(string kind, float scale, float eps)
    {
        ...
        double[] expected = kind == "Rotation" ? new[]
        {
            1.0,
            1.0
        }
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                double av = 0;
                for (int k = 0; k < n; k++)
                    av += (a[i, k] / (double)scale) * d.V[k, j];
                double vd = d.V[i, j] * (d.D[j].Real / (double)scale);
                if (d.D[j].Imag > 0)
                    vd -= d.V[i, j + 1] * (d.D[j].Imag / (double)scale);
                if (d.D[j].Imag < 0)
                    vd -= d.V[i, j - 1] * (d.D[j].Imag / (double)scale);
                error += (av - vd) * (av - vd);
        ...
        foreach (string algorithm in new[]
        {
            ...
            foreach (var shape in new[]
            {
                ...
                foreach (string kind in new[]
                {
                    ...
                    {
                        if (algorithm == "GramSchmidt" && shape.Item1 < shape.Item2)
                            continue;
                        yield return new object[]
                        {
                            algorithm,
            ...
            case "QL":
                {
                    ...
                    actual = Product(Work(q), Work(l));
                    Orthonormal(q);
                    Band(l, m, Math.Max(0, n - m));
                    break;
                }
            ...
            case "RQ":
                {
                    ...
                    actual = Product(Work(r), Work(q));
                    Orthonormal(q, false);
                    Band(r, Math.Max(0, m - n), n);
                    break;
                }
            ...
            case "Polar":
                {
                    ...
                    actual = Product(Work(u), Work(p));
                    Relative(Work(p), Adjoint(Work(p)));
                    // A partial isometry satisfies U U^H U = U even for rectangular, rank-deficient inputs.
                    var w = Work(u);
                    Relative(w, Product(Product(w, Adjoint(w)), w));
                    break;
                }
            ...
            default:
                {
                    ...
                    for (int i = 1; i < s.Length; i++)
                        Assert.True(s[i - 1] >= s[i]);
                    var inverse = Work(SVD.PseudoInverse(u, s, v));
                    var ap = Product(expected, inverse);
                    var pa = Product(inverse, expected);
                    Relative(expected, Product(ap, expected), 2e-4);
                    Relative(inverse, Product(pa, inverse), 2e-4);
                    Relative(ap, Adjoint(ap), 2e-4);
                    Relative(pa, Adjoint(pa), 2e-4);
                    break;
                }
        }
        ...
        foreach (string algorithm in new[]
        {
            ...
            foreach (int n in new[]
            {
                ...
                foreach (string kind in new[]
                {
                    ...
                    {
                        if ((algorithm is "LDU" or "Diagonal") && (kind is "Zero" or "RankOne"))
                            continue;
                        yield return new object[]
                        {
                            algorithm,
            ...
            case "LU":
                {
                    ...
                    expected = Rows(expected, p);
                    actual = Product(Work(l), Work(u));
                    Band(l, n, 0);
                    Band(u, 0, n);
                    break;
                }
            ...
            case "LDU":
                {
                    ...
                    expected = Rows(expected, p);
                    actual = Product(Product(Work(l), Diagonal(d)), Work(u));
                    Band(l, n, 0);
                    Band(u, 0, n);
                    break;
                }
            ...
            case "Diagonal":
                {
                    ...
                    actual = Product(Work(b), Diagonal(d));
                    break;
                }

            case "Hessenberg":
                {
                    var (p, h) = Hessenberg.Decompose(a);
                    Orthonormal(p);
                    Band(h, 1, n);
                    actual = Product(Product(Work(p), Work(h)), Adjoint(Work(p)));
                    break;
                }

            case "Arnoldi":
                {
                    var (q, h) = Arnoldi.Decompose(a);
                    Orthonormal(q);
                    Band(h, 1, n);
                    actual = Product(Product(Work(q), Work(h)), Adjoint(Work(q)));
                    break;
                }

            case "Schur":
                {
                    var (q, t) = Schur.Decompose(a);
                    Orthonormal(q);
                    Band(t, 0, n);
                    actual = Product(Product(Work(q), Work(t)), Adjoint(Work(q)));
                    Assert.Equal(Enumerable.Range(0, n).Select(i => t[i, i]), Schur.Eigenvalues(t));
                    break;
                }
            ...
            default:
                {
                    ...
                    actual = Product(Work(v), Diagonal(d));
                    expected = Product(expected, Work(v));
    ...
    public void HermitianFactorsAndReflectionsUseConjugation(int n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            Assert.Equal(0, l[i, i].Imag);
            Assert.True(l[i, i].Real > 0);
        }
        ...
        )
        {
            ...
            Orthonormal(lanczos.Q);
            Band(lanczos.T, 1, 1);
        }

        var vector = Enumerable.Range(0, n).Select(i => new Complex32(i + 1, 2 - i)).ToArray();
        var reflection = Householder.Reflection(vector);
        Orthonormal(reflection);
        var column = new Complex[n, 1];
        for (int i = 0; i < n; i++)
            column[i, 0] = (Complex)vector[i];
        var mapped = Product(Work(reflection), column);
        for (int i = 1; i < n; i++)
            Assert.True(Complex.Abs(mapped[i, 0]) <= 1e-5 * Norm(column));
    }
    ...
    public void GeneralizedSchurAndEigenvectorsSatisfyHomogeneousEquations(int n, string kind, float scale)
    {
        ...
        if (kind is "ZeroA" or "BothZero")
            a = new Complex32[n, n];
        if (kind is "ZeroB" or "BothZero")
            b = new Complex32[n, n];
        if (kind == "RankOneB")
            b = Sample(n, n, "RankOne", scale);
        if (kind == "SingularB")
        {
        ...
        var originalA = (Complex32[,])a.Clone();
        var originalB = (Complex32[,])b.Clone();
        ...
        for (int i = 0; i < n; i++)
        {
            Assert.Equal(0, t[i, i].Imag);
            Assert.True(t[i, i].Real >= 0);
        }

        var (v, alpha, beta) = GEVD.Decompose(a, b);
        var av = Product(Work(a), Work(v));
        var bv = Product(Work(b), Work(v));
        for (int j = 0; j < n; j++)
        {
    ...
    public void ComplexGsvdReconstructsBothInputsAndCompletesZeroSineColumns(int m, int p, int n)
    {
        ...
        )
        {
            ...
            if (zeroB)
                Assert.All(GSVD.GeneralizedSingularValues(d.S1, d.S2), x => Assert.True(float.IsPositiveInfinity(x)));
        }
    }
    ...
    public void IndependentDensePencilsAndKnownSpectraSurviveLargerProblems(int n)
    {
        for (int seed = 0; seed < 3; seed++)
        {
            ...
            for (int i = 0; i < n; i++)
                NumericAssert.Close((Complex)spectrum[i], values[i], 2e-4);
        }
    }
    ...
    private static void Relative(Complex[,] expected, Complex[,] actual, double tolerance = 3e-5)
    {
        ...
        for (int i = 0; i < expected.GetLength(0); i++)
            for (int j = 0; j < expected.GetLength(1); j++)
            {
                ...
                double r = Complex.Abs(expected[i, j] - actual[i, j]);
                error += r * r;
            }
        ...
        for (int i = 0; i < a.GetLength(0); i++)
            for (int j = 0; j < a.GetLength(1); j++)
                if (i - j > lower || j - i > upper)
                    Assert.Equal(new Complex32(0, 0), a[i, j]);
    }
}
