...
public class DecompositionEdgeCaseTests
{
    ...
    public void SingularFactorsAndPseudoinverseRespectRankAndScale(int rows, int columns, string kind, float scale)
    {
        ...
        for (int i = 0; i < singular.Length; i++)
        {
            ...
            if (i > 0)
                Assert.True(singular[i - 1] >= singular[i]);
        ...
        if (kind == "RankOne")
        {
            ...
            for (int i = 0; i < columns; i++)
                for (int j = 0; j < rows; j++)
                    expected[i, j] = (i + 1.0) * (j + 1) / (scale * left * right);
        ...
        foreach (int size in new[]
        {
            ...
            foreach (string kind in new[]
            {
                ...
                foreach (float scale in new[]
                {
                    ...
                    foreach (float tolerance in new[]
                    {
                        0f,
                        1e-7f,
                        1e-16f
    ...
    public void SchurHandlesDeflationRepeatedRootsAndScaling(int size, string kind, float scale, float tolerance)
    {
        ...
        for (int i = 2; i < size; i++)
            Assert.True(t[i, i - 1] == 0 || t[i - 1, i - 2] == 0);
        Assert.Equal(original.Cast<float>(), input.Cast<float>());
    }
    ...
    public void GeneralizedEigenvectorsSatisfyThePencilAndNormalization(int size, string kind, float scale)
    {
        ...
        if (kind is "Zero" or "Diagonal" or "Repeated")
        {
            ...
            for (int i = 0; i < size; i++)
                NumericAssert.Close(expected[i], actual[i], 2e-5, 2e-5);
        }
    ...
    private static void CheckSmallBlockEigenvectors(float[,] a, float[,] packed, Complex32[] alpha, float[] beta, float small, float large, int largeIndex)
    {
        for (int j = 0; j < 3; j++)
        {
            var v = new Complex[3];
            for (int i = 0; i < 3; i++)
                v[i] = alpha[j].Imag > 0 ? new Complex(packed[i, j], packed[i, j + 1]) : alpha[j].Imag < 0 ? new Complex(packed[i, j - 1], -packed[i, j]) : packed[i, j];
    ...
    public void QzKeepsItsTriangularFactorFreeOfScratchStorage(int size)
    {
        ...
        for (int i = 1; i < size; i++)
            for (int j = 0; j < i; j++)
                Assert.Equal(0, decomposition.T[i, j]);
    }
    ...
    private static float[,] Single(double[,] source, float scale)
    {
        ...
        for (int i = 0; i < result.GetLength(0); i++)
            for (int j = 0; j < result.GetLength(1); j++)
                result[i, j] = (float)(source[i, j] * scale);
        return result;
    }
    ...
    private static void Relative(double[,] expected, double[,] actual, double tolerance)
    {
        ...
        Assert.True(Math.Sqrt(error) <= tolerance * Math.Sqrt(norm), $"Relative Frobenius residual {Math.Sqrt(error / norm):G6} exceeds {tolerance:G6}.");
    }
    ...
    public void QrPreservesSmallLeadingComponents(float small)
    {
        ...
        NumericAssert.Close(new System.Numerics.Complex(small, small), complex.Q[0, 0] * complex.R[0, 0], 0, 2e-6);
    }
}
