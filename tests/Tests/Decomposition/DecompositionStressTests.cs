...
public class DecompositionStressTests
{
    [Theory]
    [InlineData(32, 1e-30f)]
    [InlineData(32, 1f)]
    [InlineData(32, 1e30f)]
    [InlineData(96, 1f)]
    ...
    public void RealRectangularKernelsProduceEconomyAndFullFactors(int m, int n, float scale)
    {
        ...
        Relative(a, Product(Work(qr.Q), Work(qr.R)));
        Orthogonal(qr.Q);
        ...
        Orthogonal(bidiagonal.U);
        Orthogonal(bidiagonal.V);
    }
    ...
    {
        var a = Sample(n, n, 719, scaleA);
        var b = Sample(n, n, 827, scaleB);
    ...
    {
        var a = Sample(n + 7, n, 1103, 1);
        var b = Sample(n + 11, n, 1201, 1);
    ...
    public void NmfReusesItsIterationWorkspaceAndStillImprovesTheApproximation()
    {
        ...
        var initial = NMF.Decompose(a, 4, 1);
        var refined = NMF.Decompose(a, 4, 100);
    ...
    private static float[,] Sample(int m, int n, int seed, float scale, bool symmetric = false)
    {
        ...
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++)
                a[i, j] = (float)(2 * random.NextDouble() - 1) * scale;
        if (symmetric)
            for (int i = 0; i < n; i++)
                for (int j = 0; j <= i; j++)
                    a[i, j] = a[j, i] = i == j ? (n + 1) * scale : a[i, j];
        return a;
    }

    private static void Similarity(float[,] a, float[,] q, float[,] t)
    {
        Relative(a, Product(Product(Work(q), Work(t)), Transpose(Work(q))));
        Orthogonal(q);
    }

    private static void Orthogonal(float[,] q)
    {
        var actual = Product(Transpose(Work(q)), Work(q));
        int n = actual.GetLength(0);
        double error = 0;
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++)
                error += Math.Pow(actual[i, j] - (i == j ? 1 : 0), 2);
    ...
    {
        var b = new double[a.Length, a.Length];
    ...
    private static System.Numerics.Complex[,] ComplexWork(Complex32[,] a)
    {
        ...
        for (int i = 0; i < a.GetLength(0); i++)
            for (int j = 0; j < a.GetLength(1); j++)
                b[i, j] = (System.Numerics.Complex)a[i, j];
        return b;
    }
    ...
    private static System.Numerics.Complex[,] Adjoint(System.Numerics.Complex[,] a)
    {
        ...
        for (int i = 0; i < a.GetLength(0); i++)
            for (int j = 0; j < a.GetLength(1); j++)
                b[j, i] = System.Numerics.Complex.Conjugate(a[i, j]);
        return b;
    }
