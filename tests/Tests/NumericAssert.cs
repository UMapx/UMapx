...
internal static class NumericAssert
{
    internal static void Close(double expected, double actual, double absolute = 2e-6, double relative = 2e-5)
    {
        ...
        Assert.True(double.IsFinite(expected) && double.IsFinite(actual) && Math.Abs(actual - expected) <= tolerance, FormattableString.Invariant($"Expected {expected:G17}; actual {actual:G17}; tolerance {tolerance:G6}."));
    }
    ...
    internal static void Close(Complex expected, Complex32 actual, double absolute = 2e-6, double relative = 2e-5)
    {
        ...
        Assert.True(double.IsFinite(error) && error <= tolerance, FormattableString.Invariant($"Expected {expected}; actual ({actual.Real:G9}, {actual.Imag:G9}); error {error:G6}."));
    }
    ...
    internal static void Close(float[] expected, float[] actual, double tolerance = 1e-4)
    {
        ...
        for (int i = 0; i < expected.Length; i++)
            Close(expected[i], actual[i], tolerance, 0);
    }
    ...
    internal static void Close(float[,] expected, float[,] actual, double tolerance = 1e-4)
    {
        ...
        for (int i = 0; i < expected.GetLength(0); i++)
            for (int j = 0; j < expected.GetLength(1); j++)
                Close(expected[i, j], actual[i, j], tolerance, 0);
    }
    ...
    internal static float[,] Matrix(int m, int n)
    {
        ...
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++)
                a[i, j] = (float)(2 * random.NextDouble() - 1);
        return a;
    }
}
