...
public class WaveletTests
{
    ...
    public void EveryBankAgreesWithIndependentPeriodicFilterConvolution(string name)
    {
        ...
        Assert.NotSame(bank.LowPass, clone.LowPass);
        Close(bank.LowPass, clone.LowPass);
        int n = 64;
        var x = Enumerable.Range(0, n).Select(i => new Complex32((float)Math.Sin(i * .37), (float)Math.Cos(i * .71))).ToArray();
        ...
        {
            var d = new WaveletDecomposition(bank, 1, normalized);
            var actual = d.Forward(x);
            var real = d.Forward(x.Select(z => z.Real).ToArray());
            foreach (int band in new[]
            {
                0,
                1
            }
            ...
            )
            {
                ...
                for (int i = 0; i < n / 2; i++)
                {
                    ...
                    for (int tap = 0; tap < h.Length; tap++)
                        sum += h[tap] * (Complex)x[((2 * i + tap - (h.Length / 2 - 1)) % n + n) % n];
                    if (normalized)
                        sum /= Math.Sqrt(2);
                    Close(sum, actual[band][i], 2e-4);
                    Close(sum.Real, real[band][i], 2e-4);
                }
            }

            // Check synthesis independently even for explicitly nonorthogonal Legendre banks.
            var restored = d.Backward(actual);
            var restoredReal = d.Backward(real);
            for (int i = 0; i < n; i++)
            {
                Complex expected = 0;
                ...
                )
                {
                    ...
                    for (int tap = 0; tap < h.Length; tap++)
                    {
                        ...
                        if (index % 2 == 1)
                        {
                            expected += h[tap] * (Complex)actual[band][index / 2];
                            expectedReal += h[tap] * real[band][index / 2];
                        }
                    }
                }
                ...
                if (normalized)
                {
                    ...
                    expectedReal *= Math.Sqrt(2);
                }

                Close(expected, restored[i], 3e-4);
                Close(expectedReal, restoredReal[i], 3e-4);
            }
        }
    }
    ...
    public static IEnumerable<object[]> InvertibleBanks()
    {
        foreach (var row in Banks())
        {
            ...
            if (name.StartsWith('L') && name != "L1")
                continue; // Explicitly documented as nonorthogonal.
            if (name is "Fbsp103" or "Fbsp105")
                continue; // Prototype filters without a supplied dual bank.
    ...
    {
        int n = 256;
        var x = Enumerable.Range(0, n).Select(i => (float)(.25 * Math.Sin(i * .37) + .15 * Math.Cos(i * .9))).ToArray();
        x[17] += 1;
    ...
    {
        var d = new WaveletDecomposition(WaveletPacket.D4, levels);
        var t = new WaveletTransform(d);
        var x = Enumerable.Range(0, 64).Select(i => (float)Math.Sin(i * .31)).ToArray();
        var z = x.Select((v, i) => new Complex32(v, (float)Math.Cos(i * .2))).ToArray();
        Close(x, t.Backward(t.Forward(x)), 2e-4);
        var rz = t.Backward(t.Forward(z));
        for (int i = 0; i < x.Length; i++)
            Close((Complex)z[i], rz[i], 3e-4);
        var a = NumericAssert.Matrix(16, 24);
        var c = MatrixTestSupport.ToComplex(a);
        for (int i = 0; i < 16; i++)
            for (int j = 0; j < 24; j++)
                c[i, j].Imag = (i - j) * .01f;
        Close(a, d.Backward(d.Forward(a)), 3e-4);
        Close(a, t.Backward(t.Forward(a)), 3e-4);
        foreach (var r in new[]
        {
            d.Backward(d.Forward(c)),
            t.Backward(t.Forward(c))
        }

        )
            for (int i = 0; i < 16; i++)
                for (int j = 0; j < 24; j++)
                    Close((Complex)c[i, j], r[i, j], 5e-4);
    }
    ...
    {
        var d = new EdgeAvoidingWaveletDecomposition(2, .1f, levels);
        var x = Enumerable.Range(0, 16).Select(i => (float)(.5 + .4 * Math.Sin(i * .31))).ToArray();
        var z = x.Select((v, i) => new Complex32(v, .125f)).ToArray();
        ...
        for (int i = 0; i < 8; i++)
            ...
            {
                a[i, j] = x[(i + j) % 16];
                c[i, j] = new(a[i, j], .125f);
            }

        Close(x, d.Backward(d.Forward(x)), 1e-4);
        Close(a, d.Backward(d.Forward(a)), 1e-4);
        var rz = d.Backward(d.Forward(z));
        var rc = d.Backward(d.Forward(c));
        for (int i = 0; i < z.Length; i++)
            Close((Complex)z[i], rz[i], 1e-4);
        for (int i = 0; i < 8; i++)
            for (int j = 0; j < 12; j++)
                Close((Complex)c[i, j], rc[i, j], 1e-4);
    }
    ...
    public void GaussianWaveletsHaveTheRequestedDerivativeShape(int n, float x)
    {
        ...
        Assert.True(double.IsFinite(anchor.Magnitude) && anchor.Magnitude > 0, "The derivative-shape reference requires a finite, nonzero normalization anchor.");
        ...
        Assert.Throws<NotSupportedException>(() => new Gaussian(n).Scaling(x));
        Assert.Throws<NotSupportedException>(() => complex.Scaling(x));
    }
    ...
    {
        double t = x, pi = Math.PI;
        var haar = new Haar();
        Close(t >= 0 && t < 1 ? 1 : 0, haar.Scaling(x));
        Close(t >= 0 && t < .5 ? 1 : t >= .5 && t < 1 ? -1 : 0, haar.Wavelet(x));
        ...
        var g = new Gabor(.25f, 1.5f, 2);
        var gv = Complex.Exp(-(t - .25) * (t - .25) / 4 - Complex.ImaginaryOne * 1.5 * (t - .25));
        Close(gv, g.Wavelet(x));
        Close(gv.Real, g.WaveletReal(x));
    ...
    {
        var d = new Meyer();
        Close(wavelet ? 4 / Math.PI : 2 / (3 * Math.PI), wavelet ? d.Wavelet(x) : d.Scaling(x), 1e-5);
    }

    public static IEnumerable<object[]> DualBanks() => WaveletTests.Banks().Where(row => ((string)row[0]).StartsWith("Bior") || ((string)row[0]).StartsWith("CDF"));
    public static IEnumerable<object[]> ReconstructionCases()
    {
        foreach (var row in DualBanks())
            foreach (bool normalized in new[]
            {
                false,
                true
    ...
    public void DualBanksReconstructEveryImpulsePhaseAndComplexSignals(string name, bool normalized, int size)
    {
        ...
        var input = Enumerable.Range(0, size).Select(i => new Complex32((float)Math.Sin(.31 * i), (float)Math.Cos(.17 * i))).ToArray();
        var restored = d.Backward(d.Forward(input));
        for (int i = 0; i < size; i++)
            Close((Complex)input[i], restored[i], 2e-5);
        var image = new Complex32[8, 16];
        for (int y = 0; y < 8; y++)
            for (int x = 0; x < 16; x++)
                image[y, x] = input[(y * 16 + x) % size];
        var result = d.Backward(d.Forward(image));
        for (int y = 0; y < 8; y++)
            for (int x = 0; x < 16; x++)
                Close((Complex)image[y, x], result[y, x], 4e-5);
    }

    [Theory, MemberData(nameof(DualBanks))]
    public void AnalysisWaveletsRejectConstantsAndHaveTheirAdvertisedMoments(string name)
    {
        ...
        if (name == "CDF97")
            moments = 4; // Nine/seven taps, four moments on each side.
    ...
    {
        using var stream = typeof(WaveletTests).Assembly.GetManifestResourceStream("UMapx.Tests.Data.meyer-hankel.json");
        using var data = JsonDocument.Parse(stream!);
        foreach (var row in data.RootElement.GetProperty("meyer").EnumerateArray())
            yield return new object[]
            {
                row.GetProperty("wavelet").GetBoolean(),
                row.GetProperty("x").GetSingle(),
                row.GetProperty("expected").GetDouble()
            };
    }
