...
public class DecompositionUnificationTests
{
    public static IEnumerable<object[]> ReferenceCases()
    {
        using var stream = typeof(DecompositionUnificationTests).Assembly.GetManifestResourceStream("UMapx.Tests.Data.decomposition-unification.json")!;
    ...
    public void SpectraAndFactorsAgreeWithIndependentLapackReferences(string name, string json)
    {
        ...
        if (operation == "SVD")
        {
            ...
            if (real) { var r = SVD.Decompose(Real(a), 100); d = (Work(r.U), r.S, Work(r.V)); }
            else { var c = SVD.Decompose(input, 100); d = (Work(c.U), c.S, Work(c.V)); }
            Close(a, Product(Product(d.u, Diag(d.s.Select(x => new Complex(x, 0)).ToArray())), Adjoint(d.v)));
            Orthonormal(d.u); Orthonormal(d.v);
            values = d.s.Select(x => new Complex(x, 0)).ToArray();
            for (int j = 0; j < values.Length; j++) Assert.True(Complex.Abs(values[j] - expected[j]) <= 3e-5 * expected[0].Magnitude, name);
        }
        else if (operation == "GEVD")
        {
            var b = ReadMatrix(item.GetProperty("b"), n, n);
            Complex[,] v;
            Complex32[] alpha; float[] beta;
            if (real)
            {
                var d = GEVD.Decompose(Real(a), Real(b)); v = UnpackEigenvectors(d.V, d.Alpha); alpha = d.Alpha; beta = d.Beta;
            }
            else
            {
                var d = GEVD.Decompose(input, Single(b)); v = Work(d.V); alpha = d.Alpha; beta = d.Beta;
            }
        ...
        {
            Complex[,] v;
            if (real) { var d = EVD.Decompose(Real(a)); v = UnpackEigenvectors(d.V, d.D); values = d.D.Select(x => (Complex)x).ToArray(); }
            else { var d = EVD.Decompose(input); v = Work(d.V); values = d.D.Select(x => (Complex)x).ToArray(); }
            Eigenvectors(a, v, values);
            if (operation == "Hermitian") { Orthonormal(v); Assert.All(values, x => Assert.Equal(0, x.Imaginary)); }
            // The same reference spectrum also checks the independent Schur entry points.
            if (real)
            {
                var d = Schur.Decompose(Real(a)); Close(a, Product(Product(Work(d.Q), Work(d.T)), Adjoint(Work(d.Q))));
                Orthonormal(Work(d.Q)); Match(expected, Schur.Eigenvalues(d.T).Select(x => (Complex)x).ToArray(), name);
            }
            else
            {
                var d = Schur.Decompose(input); Close(a, Product(Product(Work(d.Q), Work(d.T)), Adjoint(Work(d.Q))));
                Orthonormal(Work(d.Q)); Match(expected, Schur.Eigenvalues(d.T).Select(x => (Complex)x).ToArray(), name);
            }
        }
    ...
    [Theory]
    [InlineData(48, 32, false)] [InlineData(32, 48, false)]
    [InlineData(64, 64, true)] [InlineData(257, 17, true)] [InlineData(17, 257, true)]
    public void ComplexSvdPreservesKnownSpectraAndNullspaces(int m, int n, bool deficient)
    {
        int k = Math.Min(m, n);
        var u = Fourier(m, k); var v = Fourier(n, k);
        ...
        Orthonormal(Work(d.U)); Orthonormal(Work(d.V));
    ...
    public void HermitianReductionAndEvdResolveRepeatedAndSignedSpectra(int n)
    {
        ...
        for (int i = 0; i < n; i++) for (int j = 0; j < n; j++)
        {
            ...
            if (Math.Abs(i - j) > 1) Assert.Equal(default, reduction.T[i, j]);
        }
        var d = EVD.Decompose(a);
        var values = d.D.Select(x => (Complex)x).ToArray();
        Match(spectrum, values, "Hermitian spectrum");
        Eigenvectors(Work(a), Work(d.V), values); Orthonormal(Work(d.V));
        Assert.All(d.D, x => Assert.Equal(0, x.Imag));
    }

    [Theory]
    [InlineData(1e-30f, 1e30f)] [InlineData(1e30f, 1e-30f)]
    public void InterleavedHermitianComponentsKeepTheirOwnScales(float small, float large)
    {
        var a = new Complex32[5, 5];
        a[0, 0] = small; a[2, 2] = 3 * small; a[0, 2] = new(0, small); a[2, 0] = new(0, -small);
        a[1, 1] = large; a[3, 3] = 3 * large; a[1, 3] = new(large, 0); a[3, 1] = a[1, 3];
        a[4, 4] = 0;
        var d = EVD.Decompose(a);
        var expected = new[] { 0.0, (2 - Math.Sqrt(2)) * small, (2 + Math.Sqrt(2)) * small, (2 - Math.Sqrt(2)) * large, (2 + Math.Sqrt(2)) * large }.Order().ToArray();
        for (int i = 0; i < expected.Length; i++)
            Assert.True(Math.Abs(d.D[i].Real - expected[i]) <= Math.Abs(expected[i]) * 2e-5);
        Eigenvectors(Work(a), Work(d.V), d.D.Select(x => (Complex)x).ToArray()); Orthonormal(Work(d.V));
    }

    [Theory]
    [InlineData(24, false)] [InlineData(48, false)] [InlineData(24, true)]
    public void GeneralPencilsPreserveFiniteAndInfiniteEigenpairs(int n, bool singular)
    {
        var q = Fourier(n, n);
        var a0 = new Complex[n, n]; var b0 = new Complex[n, n];
        for (int i = 0; i < n; i++)
        {
            a0[i, i] = new Complex(i + 1, i % 3 - 1);
            b0[i, i] = singular && i % 4 == 0 ? 0 : 1 + i / (double)n;
            if (i + 1 < n) { a0[i, i + 1] = new Complex(.2, -.1); b0[i, i + 1] = .1; }
        }
        ...
        Orthonormal(Work(d.Q)); Orthonormal(Work(d.Z));
        var e = GEVD.Decompose(a, b);
        Homogeneous(Work(a), Work(b), Work(e.V), e.Alpha, e.Beta);
        if (singular) Assert.Equal(Enumerable.Range(0, n).Count(i => i % 4 == 0), e.Beta.Count(x => x == 0));
        else Match(Enumerable.Range(0, n).Select(i => a0[i, i] / b0[i, i]).ToArray(), e.Alpha.Select((x, i) => (Complex)x / e.Beta[i]).ToArray(), "pencil");
    }

    [Theory]
    [InlineData("GramSchmidt", false)] [InlineData("GramSchmidt", true)]
    [InlineData("Arnoldi", false)] [InlineData("Arnoldi", true)]
    [InlineData("Lanczos", false)] [InlineData("Lanczos", true)]
    public void BasisCompletionPreservesBothDomainsAtBreakdown(string algorithm, bool rankOne)
    {
        const int n = 32;
        var real = new float[n, n];
        if (rankOne) for (int i = 0; i < n; i++) for (int j = 0; j < n; j++) real[i, j] = (i + 1) * (j + 1);
        ...
        Orthonormal(Work(r.q)); Orthonormal(Work(c.q));
        ...
        Close(Work(real), realProduct); Close(Work(complex), complexProduct);
    }

    private static void Homogeneous(Complex[,] a, Complex[,] b, Complex[,] v, Complex32[] alpha, float[] beta)
    {
        var av = Product(a, v); var bv = Product(b, v);
        for (int j = 0; j < alpha.Length; j++)
        {
            double error = 0, vectorNorm = 0;
            for (int i = 0; i < alpha.Length; i++)
            { error += Math.Pow(Complex.Abs(beta[j] * av[i, j] - (Complex)alpha[j] * bv[i, j]), 2); vectorNorm += v[i, j].Magnitude * v[i, j].Magnitude; }
    ...
    private static void Eigenvectors(Complex[,] a, Complex[,] v, Complex[] values)
    {
        ...
        {
            double error = 0, vn = 0;
            for (int i = 0; i < values.Length; i++) { error += Math.Pow(Complex.Abs(av[i, j] - v[i, j] * values[j]), 2); vn += v[i, j].Magnitude * v[i, j].Magnitude; }
    ...
    private static Complex[,] UnpackEigenvectors(float[,] v, Complex32[] eigenvalues)
    {
        ...
        {
            if (eigenvalues[j].Imag <= 0) continue;
            for (int i = 0; i < v.GetLength(0); i++) { result[i, j] = new(v[i, j], v[i, j + 1]); result[i, j + 1] = Complex.Conjugate(result[i, j]); }
            j++;
        }
        return result;
    }
    private static Complex ReadValue(JsonElement x) => new(x[0].GetDouble(), x[1].GetDouble());
    private static Complex[,] ReadMatrix(JsonElement values, int m, int n) { var a = new Complex[m, n]; int k = 0; foreach (var x in values.EnumerateArray()) { a[k / n, k % n] = ReadValue(x); k++; } return a; }
    private static Complex[,] Fourier(int m, int n) { var a = new Complex[m, n]; for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) a[i, j] = Complex.FromPolarCoordinates(1 / Math.Sqrt(m), 2 * Math.PI * i * j / m); return a; }
    private static void MakeHermitian(Complex32[,] a) { for (int i = 0; i < a.GetLength(0); i++) { a[i, i] = a[i, i].Real; for (int j = 0; j < i; j++) a[j, i] = a[i, j].Conjugate; } }
    private static Complex[,] Work(Complex32[,] a) { var b = new Complex[a.GetLength(0), a.GetLength(1)]; for (int i = 0; i < a.GetLength(0); i++) for (int j = 0; j < a.GetLength(1); j++) b[i, j] = (Complex)a[i, j]; return b; }
    private static Complex[,] Work(float[,] a) { var b = new Complex[a.GetLength(0), a.GetLength(1)]; for (int i = 0; i < a.GetLength(0); i++) for (int j = 0; j < a.GetLength(1); j++) b[i, j] = a[i, j]; return b; }
    private static Complex32[,] Single(Complex[,] a) { var b = new Complex32[a.GetLength(0), a.GetLength(1)]; for (int i = 0; i < a.GetLength(0); i++) for (int j = 0; j < a.GetLength(1); j++) b[i, j] = new((float)a[i, j].Real, (float)a[i, j].Imaginary); return b; }
    private static float[,] Real(Complex[,] a) { var b = new float[a.GetLength(0), a.GetLength(1)]; for (int i = 0; i < a.GetLength(0); i++) for (int j = 0; j < a.GetLength(1); j++) b[i, j] = (float)a[i, j].Real; return b; }
    private static Complex[,] Diag(Complex[] d) { var a = new Complex[d.Length, d.Length]; for (int i = 0; i < d.Length; i++) a[i, i] = d[i]; return a; }
    ...
    private static void Close(Complex[,] a, Complex[,] b, double tolerance = 2e-5) { double error = 0; for (int i = 0; i < a.GetLength(0); i++) for (int j = 0; j < a.GetLength(1); j++) error += Math.Pow(Complex.Abs(a[i, j] - b[i, j]), 2); Assert.True(Math.Sqrt(error) <= tolerance * Norm(a), $"Relative residual {Math.Sqrt(error) / Norm(a)}"); }
