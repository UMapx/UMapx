...
public class DecompositionAuditTests
{
    ...
    public void FactorsReconstructTheInputAndSatisfyTheirStructuralContracts(string name,int n,string kind)
    {
        ...
        switch(name)
        {
            case "QR": {var d=QR.Decompose(a);actual=Product(d.Q,d.R);Orthonormal(d.Q);Band(d.R,0,n);Assert.Equal(a.Length,QR.HouseholderVectors(a).Length);break;}
            case "QL": {var d=QL.Decompose(a);actual=Product(d.Q,d.L);Orthonormal(d.Q);Band(d.L,n,0);break;}
            case "LQ": {var d=LQ.Decompose(a);actual=Product(d.L,d.Q);Orthonormal(d.Q);Band(d.L,n,0);break;}
            case "RQ": {var d=RQ.Decompose(a);actual=Product(d.R,d.Q);Orthonormal(d.Q);Band(d.R,0,n);break;}
            case "LU": {var d=LU.Decompose(a);actual=Product(Transpose(LU.PermutationMatrix(d.P)),Product(d.L,d.U));Band(d.L,n,0);Band(d.U,0,n);break;}
            case "LDU": {var d=LDU.Decompose(a);actual=Product(Transpose(LU.PermutationMatrix(d.P)),Product(Product(d.L,Diagonal(d.D)),d.U));Band(d.L,n,0);Band(d.U,0,n);break;}
            case "Diagonal": {var d=UMapx.Decomposition.Diagonal.Decompose(a);actual=Product(d.B,Diagonal(d.D));break;}
            ...
            case "LDL": {var d=LDL.Decompose(a);var u=LDL.UpperFactor(d.L);actual=Product(Product(d.L,Diagonal(d.D)),u);Close(u,Transpose(d.L));Band(d.L,n,0);break;}
            case "UDL": {var d=UDL.Decompose(a);var l=UDL.LowerFactor(d.U);actual=Product(Product(d.U,Diagonal(d.D)),l);Close(d.U,Transpose(l));Band(l,n,0);break;}
            case "Householder": {var d=Householder.Decompose(a);actual=Product(Product(d.H,d.T),Transpose(d.H));Orthonormal(d.H);Band(d.T,1,1);break;}
            default: {var d=Lanczos.Decompose(a,name=="LanczosFull");actual=Product(Product(d.Q,d.T),Transpose(d.Q));Orthonormal(d.Q);Band(d.T,1,1);break;}
        }
    ...
    public void RectangularAndRankDeficientMatricesRetainTheirInformation(string name,int m,int n,string kind)
    {
        ...
        if(name=="Bidiagonal")
        {
            var d=Bidiagonal.Decompose(a);Close(a,Product(Product(d.U,d.B),Transpose(d.V)),.002f);
            Orthonormal(d.U);Orthonormal(d.V);Band(d.B,0,1);
        }
        else
        {
            var d=SVD.Decompose(a,100);Close(a,Product(Product(d.U,Diagonal(d.S)),Transpose(d.V)),.003f);
            ...
            var p=SVD.PseudoInverse(d.U,d.S,d.V);var ap=Product(a,p);var pa=Product(p,a);
    ...
    public void GeneralizedFactorizationsSatisfyBothMatrixEquations(int n)
    {
        ...
        var qz=QZ.Decompose(a,b,1e-7f);Orthonormal(qz.Q);Orthonormal(qz.Z);
    ...
    public void GeneralizedSvdReconstructsBothRectangularInputs(int m,int p,int n)
    {
        ...
        for(int i=0;i<n;i++){Close(1,d.S1[i]*d.S1[i]+d.S2[i]*d.S2[i],.001);Close(1,GSVD.Identity(d.S1,d.S2)[i],.001);Close(d.S1[i]/d.S2[i],GSVD.GeneralizedSingularValues(d.S1,d.S2)[i],.001);}
    }

    [Theory] [InlineData(2)] [InlineData(3)] [InlineData(5)]
    public void HouseholderReflectionZerosTheTailAndIsOrthogonal(int n)
    {
        var v=Enumerable.Range(0,n).Select(i=>i+.5f).ToArray();var d=Householder.Reflection(v);
        Orthonormal(d);Close(d,Transpose(d));
        var transformed=new float[n];for(int i=0;i<n;i++)for(int j=0;j<n;j++)transformed[i]+=d[i,j]*v[j];
    ...
    {
        var a=new float[,]{{4,1,0},{0,2,0},{0,0,1}};var d=Power.Decompose(a,100);var v=d.V;
        Close(1,Math.Sqrt(v.Sum(x=>(double)x*x)));Close(0,v[1],1e-5);Close(0,v[2],1e-5);Close(Diagonal(v),Power.DiagonalMatrix(v));
    }
    ...
    public void NonnegativeRankOneFactorizationRecoversANonnegativeProduct(int m,int n)
    {
        ...
        var d=NMF.Decompose(a,1,100);Assert.All(d.W.Cast<float>(),x=>Assert.True(float.IsFinite(x)&&x>=0));Assert.All(d.H.Cast<float>(),x=>Assert.True(float.IsFinite(x)&&x>=0));
